...
namespace MonsterHunterCompanion
{
    public partial class MHCompanion : Form
    {
        ...
        private void MHCompanion_Load(object sender, EventArgs e)
        {
            ...
            {
                new Quests("Island Heat", "Hunting", "Hunt a Rathalos", 4500, 500, "Deserted Island", "Rathalos")
            };

            QGridview.DataSource = QuestList;

            string[] Ail = new string[] { "Small Roar", "Large Wind Pressure", "Poison", "Fireblight" };
            string[] Hab = new string[] { "Deserted Island", "Volcano", "Jurassic Frontier" };
            string[] Weak = new string[] { "Thunder", "Dragon", "Sleep" };
            string[] lItems = new string[] { "Rathalos Scale", "Rathalos Shell", "Rathalos Webbing", "Flame Sac", "Rath Marrow" };
            string[] hItems = new string[] { "Rathalos Scale+", "Rathalos Carapace", "Rathalos Wing", "Rath Medulla" };
            string[] gItems = new string[] { "Rathalos Shard", "Rathalos Cortex", "Conflagrant Sac", "Rathalos Fellwing" };

            List<Monsters> MonsterList;

            MonsterList = new List<Monsters>
            {
                new Monsters("Rathalos", Ail, Hab, Weak, lItems, hItems, gItems)
            };

            MonGridview.DataSource = MonsterList;

            List<PalicoArmor> PArmorList;
            List<PalicoWeapon> PWeaponList;

            PArmorList = new List<PalicoArmor>
            {
                new PalicoArmor("F Acorn", "1", 1, 5, 0, -5, 0, 10)
            };

            PArGridview.DataSource = PArmorList;

            PWeaponList = new List<PalicoWeapon>
            {
                new PalicoWeapon("F Bone Wedge", 3, 50, 50, 0, 0, "Cutting Type", "Balanced")
            };

            PWeGridview.DataSource = PWeaponList;

            List<Skills> SkillList;

            SkillList = new List<Skills>
            {
                new Skills("Attack Up (L)", "Greatly Increases Attack", "Attack", 20),
                new Skills("Attack Up (M)", "Increases Attack", "Attack", 15),
                new Skills("Attack Up (S)", "Slightly Increases Attack", "Attack", 10)
            };

            SkiGridView.DataSource = SkillList;

            string[] RathSkills = new string[] { "Tenderizer", "Attack", "Defense" };
            List<ArmorSet> ArmorList;

            ArmorList = new List<ArmorSet>
            {
                new ArmorSet("Rathalos", "3", 20, 0, -10, 0, -20, 120, 370, RathSkills)
            };

            ASGridview.DataSource = ArmorList;

            List<Decorations> DecoList;
            string[,] Deco1 = new string[2, 2] { { "Attack", "1"},
                                                 {"Defense", "1" } };
            string[,] Deco2 = new string[1, 2] { { "Bherna", "1" } };
            string[,] Deco3 = new string[1, 2] { { "Combo Plus", "1" } };

            DecoList = new List<Decorations>
            {
                new Decorations("Attack Jwl 1", "4", 1, 40, 400, Deco1),
                new Decorations("Bell Jwl 1", "4", 1, 40, 400, Deco2),
                new Decorations("Factory Jwl 1", "4", 1, 40, 400, Deco3)
            };

            DecGridview.DataSource = DecoList;
        }
        ...
        private void PWeGridview_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
